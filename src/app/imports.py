...
def _extract_hyperlink_text(cell: object) -> str:
    """Return the display text for hyperlink-like formulas."""

    text = _to_clean_string(cell)
    if not text:
        return ""

    match = re.search(r"=(?:HYPERLINK|LIEN_HYPERTEXTE)\s*\((.*)\)", text, flags=re.IGNORECASE)
    if not match:
        return text
...
def _looks_like_member_name(cell: object) -> bool:
    ...
    if not re.search(r"[A-Za-zÀ-ÖØ-öø-ÿ]", first):
        return False
    uppercase_letters = sum(1 for ch in first if ch.isalpha() and ch.isupper())
    if uppercase_letters < 2:
        return False

    second = tokens[1]
    return re.search(r"[A-Za-zÀ-ÖØ-öø-ÿ]", second) is not None
...
def _read_validation_rows(path: Path) -> list[list[object]]:
    """Load raw worksheet values preserving formulas such as hyperlinks."""
        ...
        if path.suffix.lower() == ".xls":
            ...
            for rx in range(sheet.nrows):
                ...
                for cx in range(sheet.ncols):
                    ...
                    value = cell.value
                    values.append("" if value in (None, "") else value)
                rows.append(values)
            return rows
        raise
...
def _parse_validation_multiline(raw_rows: Sequence[Sequence[object]]) -> list[Row]:
    if not raw_rows:
        return []
    ...
    for raw in raw_rows:
        values = [_to_clean_string(val) for val in list(raw)]
        if not any(values):
